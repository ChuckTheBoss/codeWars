...
wordList = ["his", "know", "male", "meal"];


function unscramble(scramble) {
    let match = [];
    letters = scramble.split("").sort().toString();
    wordList.forEach(word => {
        wordCheck = word.split("").sort().toString();
        if (letters === wordCheck) {
            match.push(word);
        };
    });
    return match;
...
console.log(unscramble("nowk"));
console.log(unscramble("amle"));

//You can't compare naked arrays in JS, apparently. using toString() fixed this. 
